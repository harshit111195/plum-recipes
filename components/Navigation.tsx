      ...
      {({ isActive: navIsActive }) => (
        <>
          ...
          {navIsActive && (
            <MotionDiv
              layoutId="nav-pill"
              className="absolute -top-0.5 w-14 h-14 bg-[#FFC244]/20 rounded-xl"
              transition={{ type: "spring", stiffness: 300, damping: 30 }}
            />
          )}
          
          <div className={`relative z-10 flex flex-col items-center space-y-1 transition-all duration-200 ${navIsActive ? 'text-[#FFC244] scale-105' : 'text-white/60'}`}>
              {icon}
...
export const Navigation: React.FC = () => {
  return (
    <nav 
      className="fixed bottom-0 left-0 right-0 z-40 h-[85px] bg-[#1A1A1A]/90 backdrop-blur-xl rounded-t-[24px] pb-[20px] border-t-2 border-white/30 shadow-[0_-4px_20px_rgba(0,0,0,0.4)]"
      role="navigation"
