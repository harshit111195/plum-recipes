    ...
    <div className="bg-brand-background min-h-screen pb-safe animate-in fade-in duration-500">
      ...
      <div className="relative z-10 mt-[35vh] bg-brand-background rounded-t-[32px] overflow-hidden min-h-screen shadow-[0_-10px_40px_rgba(0,0,0,0.3)]">
        ...
        <div className="px-6 pt-2 pb-6">
          ...
          <div className="flex justify-between items-start mb-4"><div className="flex gap-2 mb-2"><span className="px-3 py-1 bg-brand-primary/10 text-brand-primary rounded-full text-[13px] font-bold uppercase tracking-wide">{currentRecipe.difficulty}</span><span className="px-3 py-1 bg-brand-secondary/10 text-brand-secondary rounded-full text-[13px] font-bold uppercase tracking-wide">{currentRecipe.totalTimeMinutes} Min</span></div></div>
          {/* Title #FFFFFF, Description #A0A0A0 */}
          <h1 className="text-[32px] font-bold text-brand-text leading-[1.1] mb-2">{currentRecipe.title}</h1><p className="text-brand-text-secondary text-[17px] leading-relaxed">{currentRecipe.description}</p>
        </div>
        {/* Start Cooking button with helper text */}
        <div className="px-6 mb-8">
          <button onClick={startCooking} onTouchEnd={startCooking} className="w-full py-4 bg-brand-button-primary hover:bg-brand-button-primary-hover text-white font-bold text-[17px] rounded-2xl shadow-lg shadow-purple-500/30 active:scale-[0.98] transition flex flex-col items-center justify-center gap-1 cursor-pointer touch-manipulation">
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between px-6 mb-3">
            ...
            <span className="text-[12px] text-brand-text-tertiary">Swipe to see more →</span>
          </div>
          <div className="flex gap-4 overflow-x-auto px-6 pb-4 no-scrollbar">
            ...
            <div className="min-w-[140px] h-[140px] bg-brand-surface-light rounded-2xl p-4 shadow-sm flex flex-col justify-between">
              ...
              <div>
                <span className="text-[28px] font-bold text-brand-text-on-light block leading-none">{currentRecipe.caloriesApprox}</span>
                <span className="text-[15px] font-medium text-brand-text-tertiary">Calories</span>
              </div>
            </div>
            ...
            {macroData.map((m) => (
              <div key={m.name} className="min-w-[120px] h-[140px] bg-brand-surface-light rounded-2xl p-4 shadow-sm flex flex-col items-center justify-center relative">
                <div className="relative w-16 h-16 mb-2 flex items-center justify-center">
                  ...
                  <div className="absolute inset-0 flex items-center justify-center text-[11px] font-bold text-brand-text-tertiary">{m.value}%</div>
                </div>
                <div className="text-center">
                  <div className="text-[15px] font-bold text-brand-text-on-light">{m.name}</div>
                  <div className="text-[12px] font-medium text-brand-text-tertiary">{m.label}</div>
                </div>
              </div>
            ))}
            ...
            {currentRecipe.nutrition?.fiber && (
              <div className="min-w-[120px] h-[140px] bg-brand-surface-light rounded-2xl p-4 shadow-sm flex flex-col items-center justify-center">
                ...
                <div className="text-center">
                  <div className="text-[20px] font-bold text-brand-text-on-light">{currentRecipe.nutrition.fiber}</div>
                  <div className="text-[15px] font-bold text-brand-text-on-light">Fiber</div>
                  <div className="text-[12px] font-medium text-brand-text-tertiary">per serving</div>
                </div>
              </div>
            )}
          </div>
        ...
        <div className="px-6 mb-8">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-[12px] text-brand-text-tertiary">{available.length + missing.length} items</span>
          </div>
          <div className="bg-brand-surface rounded-2xl overflow-hidden shadow-sm border border-brand-divider">
            {available.length > 0 && (
              <div className="p-2">
                <div className="px-3 py-2 flex items-center justify-between">
                  <span className="text-[11px] font-bold text-brand-accent uppercase tracking-wider">✓ From Your Pantry</span>
                  <span className="text-[11px] text-brand-text-tertiary">Tap to mark as prepped</span>
                </div>
                {available.map((ing, i) => (
                  <div key={i} onClick={() => togglePrep(ing.name)} className="flex items-center p-3 hover:bg-brand-surface-secondary transition cursor-pointer rounded-xl group">
                    ...
                    <div className={`flex-1 ${prepList.includes(ing.name) ? 'opacity-30 line-through' : 'opacity-100'}`}>
                      <span className="text-[17px] text-brand-text font-medium">{ing.name}</span>
                      <div className="text-[14px] text-brand-text-secondary">{ing.amount}</div>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="p-2 bg-brand-secondary/10">
                <div className="px-3 py-2">
                  <div className="flex flex-wrap justify-between items-center gap-y-2 mb-1">
                    <span className="text-[11px] font-bold text-brand-secondary uppercase tracking-wider">Missing Ingredients</span>
                    <div className="flex gap-3 flex-wrap justify-end">
                      <button onClick={handleFindSubstitutions} className="text-brand-secondary flex items-center gap-1 active:opacity-50 text-[11px] font-bold border border-brand-secondary/30 bg-brand-background/50 px-2 py-1 rounded-lg shadow-sm">
                        {isLoadingSubst ? <Loader2 size={12} className="animate-spin" /> : <Sparkles size={12} />} Substitutions
                      </button>
                      <button onClick={handleAddToShop} className="text-brand-primary text-[11px] font-bold flex items-center gap-1 active:opacity-50 border border-brand-primary/30 bg-brand-background/50 px-2 py-1 rounded-lg shadow-sm">
                        <ShoppingBag size={12} /> Add All To Cart
                      </button>
                    </div>
                  </div>
                  <span className="text-[11px] text-brand-secondary/60">Get AI substitutions or add to your shopping list</span>
                </div>
                {substResult && (
                  <div className="mx-3 mb-2 p-3 bg-brand-surface-secondary rounded-xl text-sm text-brand-text border border-brand-divider relative animate-in zoom-in-95">
                    ...
                    <div className="flex items-center gap-1.5 font-bold text-brand-primary mb-1 text-xs uppercase"><Sparkles size={10} fill="currentColor" /> AI Suggestion</div>
                ...
                {missing.map((ing, i) => (
                  <div key={i} className="flex items-center p-3 rounded-xl">
                    ...
                    <div className="flex-1">
                      <span className="text-[17px] text-brand-text font-medium">{ing.name}</span>
                      <div className="text-[14px] text-brand-secondary/80 font-medium">{ing.amount}</div>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div className="px-6 pb-20">
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-[12px] text-brand-text-tertiary">{currentRecipe.instructions.length} steps</span>
          </div>
          <div className="bg-brand-surface p-5 rounded-2xl shadow-sm border border-brand-divider space-y-6">
            {currentRecipe.instructions.slice(0, 3).map((step, i) => (
              <div key={i} className="flex gap-4">
                <span className="flex-shrink-0 w-6 h-6 bg-brand-surface-secondary rounded-full flex items-center justify-center text-[13px] font-bold text-brand-text-secondary">{i + 1}</span>
                <p className="text-[17px] text-brand-text leading-relaxed">{step}</p>
              </div>
            ))}
            {currentRecipe.instructions.length > 3 && (
              <div className="pt-2 border-t border-brand-divider">
                <button onClick={startCooking} onTouchEnd={startCooking} className="w-full py-3 text-brand-primary font-medium text-[15px] bg-brand-background rounded-xl cursor-pointer touch-manipulation flex flex-col items-center gap-1">
                  <span>Tap to view all {currentRecipe.instructions.length} steps</span>
                  <span className="text-[11px] text-brand-text-tertiary font-normal">Opens hands-free Chef Mode with timers & voice</span>
                </button>
              </div>
            )}
          </div>
