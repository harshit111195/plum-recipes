...
export const RecipeDetail: React.FC<Props> = ({ recipe, onBack }) => {
  ...
  return (
    ...
    <div className="bg-[#0D0D0D] min-h-screen pb-safe animate-in fade-in duration-500">
      ...
      <div className="fixed top-0 left-0 right-0 z-50 pt-safe pointer-events-none">
        <div className="p-4 flex justify-between items-start">
          ...
          <div className="flex gap-2 pointer-events-auto"><button onClick={handleShare} aria-label="Share recipe" className="w-10 h-10 rounded-full bg-white/20 backdrop-blur-xl flex items-center justify-center text-white transition shadow-lg border border-white/10 active:scale-90"><Share size={18} strokeWidth={2.5} /></button><button onClick={() => toggleBookmark(currentRecipe)} aria-label={isSaved ? "Remove from saved" : "Save recipe"} className={`w-10 h-10 rounded-full backdrop-blur-xl flex items-center justify-center transition shadow-lg border border-white/10 active:scale-90 ${isSaved ? 'bg-white text-[#E84142]' : 'bg-white/20 text-white'}`}><Heart size={18} fill={isSaved ? "currentColor" : "none"} strokeWidth={2.5} /></button></div>
        </div>
      </div>
      {/* Content sheet - OPAQUE #0D0D0D background to cover hero image */}
      <div className="relative z-10 mt-[35vh] bg-[#0D0D0D] rounded-t-[32px] overflow-hidden min-h-screen shadow-[0_-10px_40px_rgba(0,0,0,0.3)]">
        ...
        <div className="px-6 pt-2 pb-6">
          ...
          <div className="flex justify-between items-start mb-4"><div className="flex gap-2 mb-2"><span className="px-3 py-1 bg-[#FFC244]/10 text-[#FFC244] rounded-full text-[13px] font-bold uppercase tracking-wide">{currentRecipe.difficulty}</span><span className="px-3 py-1 bg-[#E84142]/10 text-[#E84142] rounded-full text-[13px] font-bold uppercase tracking-wide">{currentRecipe.totalTimeMinutes} Min</span></div></div>
          {/* Title #FFFFFF, Description #A0A0A0 */}
          <h1 className="text-[32px] font-bold text-[#FFFFFF] leading-[1.1] mb-2">{currentRecipe.title}</h1><p className="text-[#A0A0A0] text-[17px] leading-relaxed">{currentRecipe.description}</p>
        </div>
        {/* Start Cooking button with helper text */}
        <div className="px-6 mb-8">
          <button onClick={startCooking} onTouchEnd={startCooking} className="w-full py-4 bg-[#7C3AED] hover:bg-[#6D28D9] text-white font-bold text-[17px] rounded-2xl shadow-lg shadow-purple-500/30 active:scale-[0.98] transition flex flex-col items-center justify-center gap-1 cursor-pointer touch-manipulation">
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between px-6 mb-3">
            <div className="flex items-center gap-2">
              <h3 className="text-[19px] font-bold text-[#FFFFFF]">Nutrition</h3>
              <button type="button" onClick={showNutritionDisclaimer} className="relative z-20 active:opacity-50 transition p-1 hover:bg-[#2A2A2A] rounded-full"><Info size={16} className="text-[#A0A0A0]" /></button>
            </div>
            <span className="text-[12px] text-[#6B6B6B]">Swipe to see more →</span>
          </div>
          <div className="flex gap-4 overflow-x-auto px-6 pb-4 no-scrollbar">
            {/* Calories card - off-white #FAFAFA background */}
            <div className="min-w-[140px] h-[140px] bg-[#FAFAFA] rounded-2xl p-4 shadow-sm flex flex-col justify-between">
              <div className="flex items-start justify-between">
                <div className="w-8 h-8 rounded-full bg-[#E84142]/20 flex items-center justify-center text-[#E84142]"><Flame size={16} fill="currentColor" /></div>
              </div>
              <div>
                <span className="text-[28px] font-bold text-[#1A1A1A] block leading-none">{currentRecipe.caloriesApprox}</span>
                <span className="text-[15px] font-medium text-[#6B6B6B]">Calories</span>
              </div>
            </div>
            {/* Macro cards - off-white #FAFAFA background */}
            {macroData.map((m) => (
              <div key={m.name} className="min-w-[120px] h-[140px] bg-[#FAFAFA] rounded-2xl p-4 shadow-sm flex flex-col items-center justify-center relative">
                <div className="relative w-16 h-16 mb-2 flex items-center justify-center">
                  ...
                  <div className="absolute inset-0 flex items-center justify-center text-[11px] font-bold text-[#6B6B6B]">{m.value}%</div>
                </div>
                <div className="text-center">
                  <div className="text-[15px] font-bold text-[#1A1A1A]">{m.name}</div>
                  <div className="text-[12px] font-medium text-[#6B6B6B]">{m.label}</div>
                </div>
              </div>
            ))}
            {/* Fiber card - off-white #FAFAFA background, shown if available */}
            {currentRecipe.nutrition?.fiber && (
              <div className="min-w-[120px] h-[140px] bg-[#FAFAFA] rounded-2xl p-4 shadow-sm flex flex-col items-center justify-center">
                <div className="w-12 h-12 rounded-full bg-[#00A67E]/20 flex items-center justify-center text-[#00A67E] mb-2 text-2xl">🌾</div>
                <div className="text-center">
                  <div className="text-[20px] font-bold text-[#1A1A1A]">{currentRecipe.nutrition.fiber}</div>
                  <div className="text-[15px] font-bold text-[#1A1A1A]">Fiber</div>
                  <div className="text-[12px] font-medium text-[#6B6B6B]">per serving</div>
                </div>
              </div>
            )}
          </div>
        ...
        <div className="px-6 mb-8">
          <div className="flex items-center justify-between mb-2">
            <h3 className="text-[19px] font-bold text-[#FFFFFF]">Ingredients</h3>
            <span className="text-[12px] text-[#6B6B6B]">{available.length + missing.length} items</span>
          </div>
          <div className="bg-[#1A1A1A] rounded-2xl overflow-hidden shadow-sm border border-[#333333]">
            {available.length > 0 && (
              <div className="p-2">
                <div className="px-3 py-2 flex items-center justify-between">
                  <span className="text-[11px] font-bold text-[#00A67E] uppercase tracking-wider">✓ From Your Pantry</span>
                  <span className="text-[11px] text-[#6B6B6B]">Tap to mark as prepped</span>
                </div>
                {available.map((ing, i) => (
                  <div key={i} onClick={() => togglePrep(ing.name)} className="flex items-center p-3 hover:bg-[#2A2A2A] transition cursor-pointer rounded-xl group">
                    <div className={`w-5 h-5 rounded-full border flex items-center justify-center mr-3 transition ${prepList.includes(ing.name) ? 'bg-[#00A67E] border-[#00A67E]' : 'border-[#6B6B6B]'}`}>
                    ...
                    <div className={`flex-1 ${prepList.includes(ing.name) ? 'opacity-30 line-through' : 'opacity-100'}`}>
                      <span className="text-[17px] text-[#FFFFFF] font-medium">{ing.name}</span>
                      <div className="text-[14px] text-[#A0A0A0]">{ing.amount}</div>
                    </div>
                  </div>
                ))}
              </div>
            )}
            {available.length > 0 && missing.length > 0 && <div className="h-[1px] bg-[#333333] mx-4" />}
            {missing.length > 0 && (
              <div className="p-2 bg-[#E84142]/10">
                <div className="px-3 py-2">
                  <div className="flex flex-wrap justify-between items-center gap-y-2 mb-1">
                    <span className="text-[11px] font-bold text-[#E84142] uppercase tracking-wider">Missing Ingredients</span>
                    <div className="flex gap-3 flex-wrap justify-end">
                      <button onClick={handleFindSubstitutions} className="text-[#E84142] flex items-center gap-1 active:opacity-50 text-[11px] font-bold border border-[#E84142]/30 bg-[#0D0D0D]/50 px-2 py-1 rounded-lg shadow-sm">
                        {isLoadingSubst ? <Loader2 size={12} className="animate-spin" /> : <Sparkles size={12} />} Substitutions
                      </button>
                      <button onClick={handleAddToShop} className="text-[#FFC244] text-[11px] font-bold flex items-center gap-1 active:opacity-50 border border-[#FFC244]/30 bg-[#0D0D0D]/50 px-2 py-1 rounded-lg shadow-sm">
                        <ShoppingBag size={12} /> Add All To Cart
                      </button>
                    </div>
                  </div>
                  <span className="text-[11px] text-[#E84142]/60">Get AI substitutions or add to your shopping list</span>
                </div>
                {substResult && (
                  <div className="mx-3 mb-2 p-3 bg-[#2A2A2A] rounded-xl text-sm text-[#FFFFFF] border border-[#333333] relative animate-in zoom-in-95">
                    <button onClick={() => setSubstResult(null)} className="absolute right-2 top-2 opacity-50 hover:opacity-100 text-[#A0A0A0]"><X size={14} /></button>
                    <div className="flex items-center gap-1.5 font-bold text-[#FFC244] mb-1 text-xs uppercase"><Sparkles size={10} fill="currentColor" /> AI Suggestion</div>
                    <p className="leading-relaxed text-[#A0A0A0]">{substResult}</p>
                  </div>
                )}
                {missing.map((ing, i) => (
                  <div key={i} className="flex items-center p-3 rounded-xl">
                    <div className="w-5 h-5 rounded-full border-2 border-[#E84142] mr-3 flex items-center justify-center bg-[#0D0D0D] text-[#E84142]"><Plus size={12} strokeWidth={3} /></div>
                    <div className="flex-1">
                      <span className="text-[17px] text-[#FFFFFF] font-medium">{ing.name}</span>
                      <div className="text-[14px] text-[#E84142]/80 font-medium">{ing.amount}</div>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div className="px-6 pb-20">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-[19px] font-bold text-[#FFFFFF]">Method</h3>
            <span className="text-[12px] text-[#6B6B6B]">{currentRecipe.instructions.length} steps</span>
          </div>
          <div className="bg-[#1A1A1A] p-5 rounded-2xl shadow-sm border border-[#333333] space-y-6">
            {currentRecipe.instructions.slice(0, 3).map((step, i) => (
              <div key={i} className="flex gap-4">
                <span className="flex-shrink-0 w-6 h-6 bg-[#2A2A2A] rounded-full flex items-center justify-center text-[13px] font-bold text-[#A0A0A0]">{i + 1}</span>
                <p className="text-[17px] text-[#FFFFFF] leading-relaxed">{step}</p>
              </div>
            ))}
            {currentRecipe.instructions.length > 3 && (
              <div className="pt-2 border-t border-[#333333]">
                <button onClick={startCooking} onTouchEnd={startCooking} className="w-full py-3 text-[#FFC244] font-medium text-[15px] bg-[#0D0D0D] rounded-xl cursor-pointer touch-manipulation flex flex-col items-center gap-1">
                  <span>Tap to view all {currentRecipe.instructions.length} steps</span>
                  <span className="text-[11px] text-[#6B6B6B] font-normal">Opens hands-free Chef Mode with timers & voice</span>
                </button>
              </div>
            )}
          </div>
